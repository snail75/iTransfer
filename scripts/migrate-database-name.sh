#!/bin/sh
# Script to migrate database from pingvin-share.db to mediapult-transfer.db

DATA_DIR="${DATA_DIRECTORY:-./backend/data}"
OLD_DB="$DATA_DIR/pingvin-share.db"
NEW_DB="$DATA_DIR/mediapult-transfer.db"
...
if [ -f "$OLD_DB" ] && [ ! -f "$NEW_DB" ]; then
    ...
    echo "Old: $OLD_DB"
    echo "New: $NEW_DB"
    echo "Using new database file (mediapult-transfer.db)"
elif [ ! -f "$OLD_DB" ]; then
