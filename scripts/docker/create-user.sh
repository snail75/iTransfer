...
if ! getent group mediapult-transfer-group > /dev/null 2>&1; then
    addgroup -g "$PGID" mediapult-transfer-group
fi

# Check if a user with PUID exists; if not, create it
if ! id -u mediapult-transfer > /dev/null 2>&1; then
    if ! getent passwd "$PUID" > /dev/null 2>&1; then
        adduser -u "$PUID" -G mediapult-transfer-group mediapult-transfer > /dev/null 2>&1
    else
