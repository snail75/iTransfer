...
The website is now listening on `http://localhost:3000`, have fun with Mediapult Transfer 🐧!

### Installation with Portainer

1. In the **Stacks** menu, click the **Add stack** button
2. Give you stack a name (ex. pingvinshare)
3. In the web editor, paste the content of the [docker-compose](https://github.com/mediapult/mediapult-transfer/blob/main/docker-compose.yml) file.
4. Edit the external port and the environment variables (optional).
5. Click on **Deploy the stack**.

Your container is now listening on `http://localhost:<externalport>`, have fun with Mediapult Transfer 🐧!
...
Create a systemd service file at `/etc/systemd/system/mediapult-transfer-container.service`:
...
[Service]
Environment="SERVICE=prod-mediapult-transfer"
Type=simple
TimeoutStartSec=30s
...
ExecStart=/usr/bin/podman run --name $SERVICE \
  ...
  -v /opt/podman-mediapult-transfer/data:/opt/app/backend/data:Z \
  -v /opt/podman-mediapult-transfer/images:/opt/app/frontend/public/img:Z \
  -e TRUST_PROXY=true \
  -e NODE_TLS_REJECT_UNAUTHORIZED=0 \
  mediapult-transfer:latest
...
- **Volume mounts**: Update the volume paths (`/opt/podman-mediapult-transfer/data` and `/opt/podman-mediapult-transfer/images`) to match your desired data directories
...
sudo systemctl enable mediapult-transfer-container.service
sudo systemctl start mediapult-transfer-container.service
```

Check the status with:

```bash
sudo systemctl status mediapult-transfer-container.service
```
...
- [pm2](https://pm2.keymetrics.io/) for running Mediapult Transfer in the background

```bash
git clone https://github.com/mediapult/mediapult-transfer
...
npm install
npm run build
pm2 start --name="mediapult-transfer-backend" npm -- run prod
